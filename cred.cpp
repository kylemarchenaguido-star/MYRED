...
std::string cred_random_hex(size_t nhex) {
  static const char *hx = "0123456789abcdef";
  size_t nbytes = (nhex + 1) / 2; // one byte feeds two nibbles
  ...
  for (size_t i = 0; i < nhex; ++i) {
    uint8_t b = buf[i / 2];
    s[i] = hx[(i % 2) ? (b & 0xf) : (b >> 4)];
  }
  secure_zero(buf.data(), nbytes);
  return s;
}
...
std::string cred_hash_new(const std::string &plain) {
  ...
  if (!fill_random(salt, sizeof(salt))) {
    return std::string();
  }
  size_t enc_len = argon2_encodedlen(k_t_cost, k_m_cost, k_par, k_salt_len,
                                     k_tag_len, Argon2_id);
  std::string enc(enc_len, '\0');
  int rc = argon2id_hash_encoded(k_t_cost, k_m_cost, k_par, plain.data(),
                                 plain.size(), salt, sizeof(salt), k_tag_len,
                                 &enc[0], enc.size());
  if (rc != ARGON2_OK) {
    return std::string();
  }
...
#ifdef MYRED_HAVE_ARGON2
    // re derives wit th embedded salt/params; tag compare inside the lib is
    // constant time
    return argon2id_verify(stored.c_str(), plain.data(), plain.size()) ==
           ARGON2_OK;
#else
...
bool cred_needs_rehash(const std::string &stored) {
  ...
  if (sscanf(stored.c_str(), "$argon2id$v=%*u$m=%u,t=%u,p=%u$", &m, &t, &p) !=
      3) {
    return true;
  }
...
const std::string &cred_dummy() {
  ...
  static const std::string d = []() {
    std::string plain = cred_random_hex(32);
    if (plain.empty()) {
      plain.assign(32, 'x');
    } // entropy failure
    ...
    if (h.empty()) {
      h = std::string(64, '0');
    } // entropy failure: still a valid-shaped dummy
    return h;
  }();
  return d;
}
