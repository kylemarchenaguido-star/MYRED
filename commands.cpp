...
static constexpr const char *MSG_WRONGTYPE =
    "WRONGTYPE Operation against a key holding the wrong kind of value";
static constexpr const char *MSG_NOT_INT =
    "ERR value is not an integer or out of range";
// static constexpr const char *MSG_NOT_FLOAT = "ERR value is not a valid
// float";
...
static uint8_t lfu_decay(uint32_t lru) {
  ...
  if (periods) {
    counter = (periods > counter) ? 0 : (uint8_t)(counter - periods);
  }
  return counter;
}
...
// WARNING: swaps cmd[i] into the LookupKey, leaving cmd[i] empty after the
...
// A unifromly-ish random live entry from the ehole keyspace (both rehash
// tables).
...
// Pick a key to evict for the active policy, or nullptr if there's nothing to
// take (noeviction, or volatile-* policy with no TTL keys -> caller must return
// -OOM)
Entry *evict_pick_victim() {
  ...
  case P::VOLATILE_TTL:
    ...
    return g_data.heap.empty()
               ? nullptr
               : container_of(g_data.heap[0].ref, &Entry::heap_idx);
  default:
    break; // LRU / LFU: best of N sampling below
  }
...
static void setex_generic(std::vector<std::string> &cmd, Buffer *out,
                          int64_t mult) {
  int64_t ttl = 0;
...
static void do_setex(std::vector<std::string> &cmd, Buffer *out) {
  setex_generic(cmd, out, 1000);
}
static void do_psetex(std::vector<std::string> &cmd, Buffer *out) {
  setex_generic(cmd, out, 1);
}
...
static void do_getdel(std::vector<std::string> &cmd, Buffer *out) {
  ...
  std::string val =
      std::move(entry_str(ent)); // we extract the values before freeing
...
static void do_getex(std::vector<std::string> &cmd, Buffer *out) {
  ...
  if (opt == "ex") {
    if (v <= 0 || v > INT64_MAX / 1000)
      return resp_err(out, "ERR invalid expire time in getex command");
    ttl_ms = v * 1000;
  } else if (opt == "px") {
    if (v <= 0)
      return resp_err(out, "ERR invalid expire time in getex command");
    ...
    ttl_ms = v - (int64_t)get_wall_msec();
  } else {
    return resp_err(out, MSG_SYNTAX);
  }
...
static void do_mget(std::vector<std::string> &cmd, Buffer *out) {
  ...
  for (size_t i = 1; i < cmd.size(); ++i) {
    ...
    case Lookup::WRONGTYPE:
      resp_nil(out);
      break; // type mismatch -> nil, NOT error
    case Lookup::MISSING:
      resp_nil(out);
      break;
    case Lookup::OK:
...
static void do_strlen(std::vector<std::string> &cmd, Buffer *out) {
  ...
  case Lookup::WRONGTYPE:
    return resp_err(out, MSG_WRONGTYPE);
  case Lookup::MISSING:
    return resp_int(out, 0); // missing -> 0, not nil
  case Lookup::OK:
    break;
  }
  resp_int(out, (int64_t)entry_str(ent).size());
...
static void do_del(std::vector<std::string> &cmd, Buffer *out) {
  ...
  if (deleted > 0) {
    g_data.g_writes_since_save += (uint32_t)deleted;
  }
...
static bool cb_keys_collect(HNode *node, void *arg) {
  ...
  Entry *ent = container_of(node, &Entry::node);
  if (glob_match(ctx->pattern->data(), ctx->pattern->size(), ent->key.data(),
                 ent->key.size())) {
...
static void expire_generic(std::vector<std::string> &cmd, Buffer *out,
                           int64_t mult) {
  int64_t ttl = 0;
...
static void ttl_generic(std::vector<std::string> &cmd, Buffer *out,
                        int64_t div) {
  LookupKey key;
  key.key.swap(cmd[1]);
  key.node.hcode = str_hash((uint8_t *)key.key.data(), key.key.size());
...
// PERSIST key  -> remove the TTL but keep the key. 1 if a TTL was removed, else
// 0
...
void audit_open(const std::string &path) {
  if (g_audit_fd >= 0 && g_audit_fd != STDERR_FILENO) {
    close(g_audit_fd);
  }
  g_audit_fd = -1;
  if (path.empty()) {
  ...
  if (fd < 0) {
    g_audit_last_error = std::string("open ") + path + ": " + strerror(errno);
    return;
  }
  g_audit_fd = fd;
}
...
void audit_event(const char *event, const Conn *conn,
                 const std::string &extra) {
  audit_write(event, conn ? conn->peer : "-",
              (conn && conn->user) ? conn->user->name : std::string("-"),
              extra);
}
...
struct AuthJob {
  ...
  std::vector<std::string>
      hashes;              // snapshot, the worker does not touch g_config.users
...
static void auth_complete(AuthJob *job) {
  ...
  Conn *c = (job->fd >= 0 && (size_t)job->fd < g_data.fd2conn.size())
                ? g_data.fd2conn[job->fd]
                : nullptr;
  // conn died mid-verify
  if (!c || c->id != job->conn_id) {
    delete job;
    return;
  }
  ...
  bool ok = job->ok && job->user_known && it != g_config.users.end() &&
            it->second.enable;
  if (ok) {
    c->user = &it->second;
    c->failed_attemps = 0;
    // rehash on auth: swap in the upgrade only if the exact old entry still
    // exists
...
static void auth_verify_job(void *arg) {
  ...
  if (!job->pass.empty()) {
    secure_zero(&job->pass[0], job->pass.size());
  }
...
static void do_bgsave(std::vector<std::string> &cmd, Buffer *out) {
  ...
  rdb_save_background();
  resp_str(out, "Background saving started",
           sizeof("Background saving started") - 1);
}
...
info_add(std::string &out, const char *fmt, ...) {
  char line[512];
  va_list ap;
  va_start(ap, fmt);
  ...
  // n is what vsnprintf WANTED to write; clamp so a long value truncates, never
  // over-reads
...
static void info_server(std::string &o) {
  const uint64_t uptime_s =
      (get_monotonic_msec() - g_data.g_server_start_ms) / 1000;
  o += "# Server\r\n";
  o += "version:1.0.0\r\n";
...
static void info_replication(std::string &o) {
  ...
  // the link fields belong beside the role, not appended after the backlog
  // stats
  if (replica) {
...
// Table order is output order, the same invariant k_config_table has for the
// file order
static const InfoSection k_info_sections[] = {
    {"server", info_server},           {"clients", info_clients},
    {"memory", info_memory},           {"stats", info_stats},
    {"keyspace", info_keyspace},       {"persistence", info_persistence},
...
static void do_info(std::vector<std::string> &cmd, Buffer *out) {
  ...
  for (size_t i = 1; i < cmd.size(); ++i) {
    ...
    if (arg_ieq(cmd[i], "all") || arg_ieq(cmd[i], "everything") ||
        arg_ieq(cmd[i], "default")) {
      all = true;
      continue;
    }
    for (size_t s = 0; s < k_info_nsections; ++s) {
      if (arg_ieq(cmd[i], k_info_sections[s].name)) {
        want[s] = true;
        break;
      }
    }
...
void do_lpop(std::vector<std::string> &cmd, Buffer *out) {
  ...
  std::string val;
  if (!deque_pop_front(&entry_deque(ent), &val)) {
    return resp_nil(out);
  }
...
void do_rpop(std::vector<std::string> &cmd, Buffer *out) {
  ...
  std::string val;
  if (!deque_pop_back(&entry_deque(ent), &val)) {
    return resp_nil(out);
  }
  // if list is now empty, delete the key
  if (entry_deque(ent).count == 0) {
...
void do_lset(std::vector<std::string> &cmd, Buffer *out) {
  ...
  case Lookup::WRONGTYPE:
    return resp_err(out, "WRONGTYPE wrong type");
  case Lookup::MISSING:
    return resp_err(out, "ERR no such key");
  case Lookup::OK:
    break;
  }
...
void do_linsert(std::vector<std::string> &cmd, Buffer *out) {
  ...
  if (entry_deque(ent).count == entry_deque(ent).cap) {
    deque_grow(&entry_deque(ent));
  }

  deque_open_gap(&entry_deque(ent), insert_idx);
  std::string &slot =
      entry_deque(ent).buf[deque_phys(&entry_deque(ent), insert_idx)];
  slot = value;
  ...
  if (from_tail) {
    ...
    while (i > 0) {
      ...
      if (*deque_get(&entry_deque(ent), i) == value) {
        entry_deque(ent).elem_bytes -=
            deque_get(&entry_deque(ent), i)->capacity();
...
void do_ltrim(std::vector<std::string> &cmd, Buffer *out) {
  ...
  for (int64_t i = 0; i < start; ++i) {
    std::string &slot =
        entry_deque(ent).buf[deque_phys(&entry_deque(ent), (size_t)i)];
    entry_deque(ent).elem_bytes -= slot.capacity();
  ...
  entry_deque(ent).head = deque_phys(
      &entry_deque(ent), (size_t)start); // compute before changing count
...
static void cb_scan(HNode *node, void *arg) {
  ...
  if (entry_has_ttl(ent) &&
      g_data.heap[ent->heap_idx].val <= get_monotonic_msec()) {
    return;
  }

  // MATCH filter (if a pattern was given)
  if (ctx->pattern && !glob_match(ctx->pattern->data(), ctx->pattern->size(),
                                  ent->key.data(), ent->key.size())) {
    return;
  }
...
static void do_scan(std::vector<std::string> &cmd, Buffer *out) {
  int64_t cursor = 0;
  if (!str2int(cmd[1], cursor)) {
    return resp_err(out, "ERR invalid cursor");
  }
  if (cursor < 0) {
    return resp_err(out, "ERR invalid cursor");
  }
...
static void do_hset(std::vector<std::string> &cmd, Buffer *out) {
  ...
  int64_t added = 0;
  for (size_t i = 2; i + 1 < cmd.size(); i += 2) {
    if (hash_set(&entry_hash(ent), cmd[i], cmd[i + 1])) {
      added++;
    }
  }
  mem_reaccount(ent);
...
static void do_hdel(std::vector<std::string> &cmd, Buffer *out) {
  ...
  int64_t removed = 0;
  for (size_t i = 2; i < cmd.size(); ++i) {
    if (hash_del(&entry_hash(ent), cmd[i])) {
      removed++;
    }
  }
...
static void h_collect_reply(std::vector<std::string> &cmd, Buffer *out,
                            int mode) {
  Entry *ent;
  switch (lookup_entry(cmd[1], T_HASH, false, &ent)) {
  case Lookup::WRONGTYPE:
...
static void do_hgetall(std::vector<std::string> &cmd, Buffer *out) {
  h_collect_reply(cmd, out, 0);
}
static void do_hkeys(std::vector<std::string> &cmd, Buffer *out) {
  h_collect_reply(cmd, out, 1);
}
static void do_hvals(std::vector<std::string> &cmd, Buffer *out) {
  h_collect_reply(cmd, out, 2);
}
...
static void do_hmget(std::vector<std::string> &cmd, Buffer *out) {
  ...
  for (size_t i = 2; i < cmd.size(); ++i) {
    HashNode *hn =
        (r == Lookup::OK) ? hash_get(&entry_hash(ent), cmd[i]) : nullptr;
    if (hn) {
      resp_str(out, hn->value.data(), hn->value.size());
    } else {
      resp_nil(out);
...
static void do_hsetnx(std::vector<std::string> &cmd, Buffer *out) {
  ...
  // already there
  if (hash_get(&entry_hash(ent), cmd[2])) {
    return resp_int(out, 0);
  }
...
static void do_hincrby(std::vector<std::string> &cmd, Buffer *out) {
  ...
  if ((incr > 0 && cur > INT64_MAX - incr) ||
      (incr < 0 && cur < INT64_MIN - incr)) {
    return resp_err(out, "ERR increment or decrement would overflow");
  }
  cur += incr;
...
static void cb_hscan(HNode *node, void *arg) {
  ...
  HashNode *hn = container_of(node, &HashNode::node);
  if (c->pattern && !glob_match(c->pattern->data(), c->pattern->size(),
                                hn->field.data(), hn->field.size())) {
    return;
  }
...
static void do_hscan(std::vector<std::string> &cmd, Buffer *out) {
  ...
  uint64_t next =
      hm_scan(&entry_hash(ent), (uint64_t)cursor, count, cb_hscan, &ctx);

  std::string cur = std::to_string(next);
  resp_str(out, cur.data(), cur.size());
  resp_arr(out, (uint32_t)items.size());
  for (const auto &s : items) {
    resp_str(out, s.data(), s.size());
  }
}
...
// sharead rename: nx=true -> fail(0) if dest exists. returns 1 ok, 0
// dest-exits, -1 no source
...
// EXPIREAT (mult=10000, seconds) / PEXPIREAT (multi=1, ms): absoulute wall
// clock expiry
static void expireat_generic(std::vector<std::string> &cmd, Buffer *out,
                             int64_t mult) {
  int64_t when = 0;
  if (!str2int(cmd[2], when)) {
    return resp_err(out, "ERR invalid expire time");
  }
...
static void do_expireat(std::vector<std::string> &cmd, Buffer *out) {
  expireat_generic(cmd, out, 1000);
}
static void do_pexpireat(std::vector<std::string> &cmd, Buffer *out) {
  expireat_generic(cmd, out, 1);
}

// Set commands

// Collects all member strings from a set's HMap (used by multi-key ops and bulk
// commands)
...
static bool cb_members_emit(HNode *node, void *arg) {
  Buffer *out = (Buffer *)arg;
  resp_str(out, container_of(node, &SetNode::node)->member.data(),
           container_of(node, &SetNode::node)->member.size());
  return true;
}

// Non-destructive — takes const ref, makes an internal copy of the key for
// hashing. Use for all read-only lookups (GET, HGET, EXPIRE, etc.)
...
static bool sinter_impl(std::vector<std::string> &cmd, size_t start,
  ...
  for (size_t i = 1; i < sets.size(); ++i) {
    if (hm_size(&entry_set(sets[i])) <
        hm_size(&entry_set(sets[smallest_idx]))) {
      smallest_idx = i;
    }
  }
  ...
  for (auto &m : candidates) {
    ...
    for (size_t i = 0; i < sets.size(); ++i) {
      ...
      if (!set_is_member(&entry_set(sets[i]), m)) {
        in_all = false;
        break;
      }
    }
...
static bool sunion_impl(std::vector<std::string> &cmd, size_t start,
                        std::vector<std::string> &result) {
  bool wt = false;
  for (size_t i = start; i < cmd.size(); ++i) {
    Entry *e = lookup_set_ro(cmd[i], &wt);
...
static bool sdiff_impl(std::vector<std::string> &cmd, size_t start,
                       std::vector<std::string> &result) {
  bool wt = false;
  ...
  for (auto &m : candidates) {
    bool found = false;
    for (Entry *e : others) {
      if (set_is_member(&entry_set(e), m)) {
        found = true;
        break;
      }
    }
...
static void do_sadd(std::vector<std::string> &cmd, Buffer *out) {
  ...
  int64_t added = 0;
  for (size_t i = 2; i < cmd.size(); ++i) {
    if (set_add(&entry_set(ent), std::move(cmd[i]))) {
      ++added;
    }
  }
  if (added > 0) {
...
static void do_srem(std::vector<std::string> &cmd, Buffer *out) {
  ...
  int64_t removed = 0;
  for (size_t i = 2; i < cmd.size(); ++i) {
    if (set_remove(&entry_set(ent), cmd[i])) {
      ++removed;
    }
  }
...
// teardown: called from conn_destroy so propagate() never dereferemces a freed
// conn.
...
// Settle a deferred wait. Always an integer, never an error
static void wait_finish(Conn *conn, size_t reached) {
  conn->wait_pending = false;
  g_data.waiters.erase(conn);
  resp_int(&conn->outgoing, (int64_t)reached);
  conn_resume(conn); // the same drain-and-flush path async AUTH already uses
}

// Third sink of the write stream. A replica is a subsacriber that never
// unsubscribes, same as PUBLISH
...
static void propagate(const char *bytes, size_t len) {
  ...
  if (!g_data.replicas.empty()) {
    repl_feed_replicas(bytes, len);
  }
}

// called on every ack and every timer tick; returns immediately in the normal
void wait_try_resume() {
  if (g_data.waiters.empty()) {
    return;
  }
  const uint64_t now = get_monotonic_msec();
  // collect first
  std::vector<Conn *> done;
  for (Conn *c : g_data.waiters) {
    if (replicas_acked(c->wait_target) >= (size_t)c->wait_numreplicas ||
        (c->wait_deadline_ms && now >= c->wait_deadline_ms)) {
      done.push_back(c);
    }
  }
  for (Conn *c : done) {
    wait_finish(c, replicas_acked(c->wait_target));
  }
}

// teardown: waiters holds raw Conn* that wait_try_resume dereferences
void wait_remove_conn(Conn *conn) {
  if (!conn->wait_pending) {
    return;
  }
  conn->wait_pending = false;
  g_data.waiters.erase(conn);
...
static void do_replconf(std::vector<std::string> &cmd, Buffer *out,
                        Conn *conn) {
  (void)conn;
  // ACK is one option that must NOT be answered: it arrives on the replication
  // link where any reply would be parsed by the replica as part of the write
  // scream
  if (cmd.size() >= 2 && arg_ieq(cmd[1], "ack")) {
    long off = 0;
    if (cmd.size() >= 3 && parse_int_strict(cmd[2].c_str(), &off) && off >= 0) {
      conn->ack_offset = (uint64_t)off;
      conn->ack_time_ms = get_monotonic_msec();
      wait_try_resume(); // this acl may be the one a blocked wait needed
    }
    return;
  }
  if (cmd.size() >= 3 && arg_ieq(cmd[1], "listening-port")) {
    long p = 0;
    if (parse_int_strict(cmd[2].c_str(), &p) && p > 0 && p < 65536) {
      conn->replica_port = (int)p;
    }
  }
  resp_ok(out);
}
...
static void do_psync(std::vector<std::string> &cmd, Buffer *out, Conn *conn) {
  ...
  if (g_data.g_loading) {
    return resp_err(out, "ERR PSYNC is not valid during loading");
  }
  if (conn->is_replica) {
  ...
  if (cmd[1] != "?" && !g_data.repl_backlog.empty()) {
    ...
    if (id_ok && off_ok && want <= next &&
        (next - want) <= g_data.repl_backlog_histlen) {
      ...
      fprintf(
          stderr,
          "replication: partial resync for %s from offset %llu (%llu bytes)\n",
          conn->peer.c_str(), (unsigned long long)want,
  ...
  g_data.sync_full++;
  fprintf(stderr,
          "replication: replica %s attached at offset %llu (%zu byte image)\n",
...
static void do_spop(std::vector<std::string> &cmd, Buffer *out) {
  ...
  if (feed) {
  // not cmd[1]: lookup_entry() swapped 
    synth = {"srem", ent->key};
  }
  ...
  if (cmd.size() == 2) {
    ...
    if (feed) {
      synth.push_back(std::move(sn->member));
    }
    delete sn;
  } else {
    int64_t count = 0;
...
static void do_srandmember(std::vector<std::string> &cmd, Buffer *out) {
  ...
  if (count > 0) {
    // distinct members up to min(count, size): pop k random nodes, emit,
    // reinsert
...
static void do_sscan(std::vector<std::string> &cmd, Buffer *out) {
  ...
  ScanCtx ctx{&items, pattern};
  uint64_t next =
      hm_scan(&entry_set(ent), (uint64_t)cursor, count, cb_sscan, &ctx);
  std::string cur = std::to_string(next);
  resp_str(out, cur.data(), cur.size());
  resp_arr(out, (uint32_t)items.size());
  for (auto &s : items) {
...
static void set_store_result(const std::string &dkey,
                             std::vector<std::string> &result, Buffer *out) {
  if (result.empty()) {
    LookupKey lk;
    lk.key = dkey;
    lk.node.hcode = str_hash((const uint8_t *)lk.key.data(), lk.key.size());
  ...
  for (auto &m : result) {
    set_add(&entry_set(dest), std::move(m));
  }
  mem_reaccount(dest);
  g_data.g_writes_since_save++;
...
// SINTERSTORE dest key [key...] -- compute first, then store, so dest can be a
// source
static void do_sinterstore(std::vector<std::string> &cmd, Buffer *out) {
  std::vector<std::string> result;
  if (!sinter_impl(cmd, 2, result)) {
    return resp_err(out, "WRONGTYPE wrong type");
  }
  set_store_result(cmd[1], result, out);
}

// SUNIONSTORE dest key [key...] -- compute first, then store, so dest can be a
// source
static void do_sunionstore(std::vector<std::string> &cmd, Buffer *out) {
  std::vector<std::string> result;
  if (!sunion_impl(cmd, 2, result)) {
    return resp_err(out, "WRONGTYPE wrong type");
  }
  set_store_result(cmd[1], result, out);
}

// SDIFFSTORE dest key [key...] -- compute first, then store, so dest can be a
// source
static void do_sdiffstore(std::vector<std::string> &cmd, Buffer *out) {
  std::vector<std::string> result;
  if (!sdiff_impl(cmd, 2, result)) {
    return resp_err(out, "WRONGTYPE wrong type");
  }
  set_store_result(cmd[1], result, out);
}
...
static void do_smove(std::vector<std::string> &cmd, Buffer *out) {
  ...
  if (!dst_ent) {
    ...
    dst_ent->key = cmd[2];
    dst_ent->node.hcode =
        str_hash((const uint8_t *)dst_ent->key.data(), dst_ent->key.size());
...
static void do_config(std::vector<std::string> &cmd, Buffer *out) {
  ...
  if (sub == "rewrite") {
    ...
    return config_rewrite(g_config.config_path.c_str())
               ? resp_ok(out)
  ...
  if (sub == "get") {
    if (cmd.size() < 3) {
      return resp_err(out, "ERR wrong number of arguments for 'config|get'");
    }
    ...
    for (std::string &n : names) {
      if (!glob_match(param.data(), param.size(), n.data(), n.size())) {
        continue;
      }
      std::string v;
      if (config_get_value(n, v)) {
        kv.emplace_back(std::move(n), std::move(v));
      }
    }
  ...
  if (sub == "set") {
    if (cmd.size() < 4) {
      return resp_err(out, "ERR wrong number of arguments of 'config|set'");
    }
    std::string err;
    std::string p = cmd[2];
    for (char &c : p) {
    ...
    if (p == "rename-command") {
      return resp_err(
          out, "ERR 'rename-command' can only be set in the config file");
    }
    if (config_is_boot_only(p)) {
      return resp_err(out,
                      "ERR TLS parameters can only be set in the config file");
    }
    CfgResult res = config_apply(cmd[2], {cmd[3]}, err);
    if (res == CfgResult::BADVALUE) {
      return resp_err(out, ("ERR " + err).c_str());
    }
    if (res == CfgResult::UNKNOWN) {
      return resp_err(
          out, ("ERR Unknown config parameter '" + cmd[2] + "'").c_str());
    }
    return resp_ok(out);
  }
...
// Non creating, type-agnostic lookup with lazy expiry. Does not stamp LRU
// access (object idletime must report idle time, not reset it) same as
// lookup_entry but wwithout the stamp, just a helper
...
static const char *object_encoding(Entry *ent) {
  ...
  case T_STR: {
    int64_t tmp;
    return str2int(entry_str(ent), tmp) ? "int" : "raw";
  }
  case T_DLIST:
    return "deque";
  case T_HASH:
...
static void do_memory(std::vector<std::string> &cmd, Buffer *out) {
  ...
  if (sub == "usage") {
    if (cmd.size() >= 4) {
      ...
      if (opt != "samples" || cmd.size() < 5) {
        return resp_err(out, MSG_SYNTAX);
      }
      int64_t n = 0;
      if (!str2int(cmd[4], n) || n < 0) {
        return resp_err(out, MSG_SYNTAX);
      }
    }
  ...
  if (sub == "doctor") {
    ...
    if (sweep == g_data.used_memory) {
      n = snprintf(buf, sizeof(buf),
                   "Can't find any memory problem. used_memory=%zu matches a "
                   "full sweep.",
                   g_data.used_memory);
    } else {
      n = snprintf(buf, sizeof(buf),
                   "Accounting drift detected: counter=%zu sweep=%zu "
                   "(delta=%zd) - a write handler "
                   "is likely a missing a mem_reaccount.",
                   g_data.used_memory, sweep,
  ...
  if (sub == "stats") {
    resp_arr(out, 10);
    resp_str(out, "used_memory", 11);
    resp_int(out, (int64_t)g_data.used_memory);
    resp_str(out, "keys.count", 10);
    resp_int(out, (int64_t)hm_size(&g_data.db));
    resp_str(out, "maxmemory", 9);
    resp_int(out, (int64_t)g_config.maxmemory);
    resp_str(out, "maxmemory.policy", 16);
    {
      const char *p = maxmemory_policy_name(g_config.maxmemory_policy);
      resp_str(out, p, strlen(p));
    }
    resp_str(out, "evicted.keys", 12);
    resp_int(out, (int64_t)g_data.evicted_keys);
    return;
  }
...
static void do_object(std::vector<std::string> &cmd, Buffer *out) {
  if (cmd.size() < 3) {
    return resp_err(out, "ERR wrong nubmer of arguments");
  }
  std::string sub = cmd[1];
  for (char &c : sub) {
    c = (char)tolower((unsigned char)c);
  ...
  if (sub == "freq") {
    if (!policy_is_lfu()) {
      return resp_err(out, "ERR An LFU maxmemory policy is not selected, "
    ...
    if (opt == "persist") {
      ...
      if (str2int(cmd[3], v)) {
        ...
        if (abs_ms <= now) {
          ...
          int n = snprintf(ts, sizeof(ts), "%lld", (long long)abs_ms);
          aof_encode(frame,
                     {"PEXPIREAT", cmd[1], std::string_view(ts, (size_t)n)});
        }
      } else {
        aof_encode(frame, cmd);
      }
...
// Evict keys unitl we're back under maxmemory. Returns false if we can't get
// under (noeviction, or a volatile-* policy with nothing evictable) -> caller
// return -OOM. NEW, if the batcj runs out while victims remain, arms
// g_evict_pending and return true
static bool free_memory_if_needed() {
  // unlimited
  if (g_config.maxmemory == 0) {
  ...
  while (g_data.used_memory > g_config.maxmemory && --attempts > 0) {
    ...
    // CRITICAL: log an explicit DEL so AOF replay / replicas don't resurrect
    // the key. aof_feed already handles the rewrite dual-buffer; gate on
    // aof_eanble + not loading.
  ...
  g_data.g_evict_pending =
      true; // batch exhausted, victims remain: continue on ticks
  return true;
}

// one bounded eviction batch per event-loop tick while over maxmemory
void evict_tick() {
  if (g_data.g_evict_pending) {
    (void)free_memory_if_needed();
  }
}

enum class KeySpec : uint8_t { NONE, FIRST, ALL_FROM_1, STRIDE2_FROM_1 };

using CmdFn = void (*)(std::vector<std::string> &, Buffer *);
using KeyResolver = void (*)(const std::vector<std::string> &cmd,
...
struct CmdSpec {
  ...
  bool aof_self =
      false; // non deterministic: handler feeds a deterministic form itself
...
static void cmd_collect_keys(const CmdSpec &spec,
                             const std::vector<std::string> &cmd,
                             std::vector<std::string_view> &keys) {
  if (spec.key_resolver) {
    return spec.key_resolver(cmd, keys);
  }
  switch (spec.keys) {
  case KeySpec::NONE:
    break;
  case KeySpec::FIRST:
    if (cmd.size() > 1) {
      keys.emplace_back(cmd[1]);
    }
    break;
  case KeySpec::ALL_FROM_1:
    for (size_t i = 1; i < cmd.size(); ++i) {
      keys.emplace_back(cmd[i]);
    }
    break;
...
static const char *acl_check(const User *u, const std::string &name,
                             ...
                             const std::vector<std::string> &cmd) {
  if (!u || !u->enable) {
    return "NOPERM this user is disable";
  }

  // (a) command permission: explicit +cmd/-cmd override wins, else category
  // grant
  auto ov = u->cmd_overrides.find(name);
  bool ok = (ov != u->cmd_overrides.end())
                ? ov->second
                : ((spec.acl_cats & u->allow_cats) != 0);
  if (!ok) {
    return "NOPERM this user has no permissions to run this command";
  }

  // (b) key patterns (skipped for all_keys users and no key commands)
  if (!u->all_keys) {
    const char *kdeny =
        "NOPERM no permissions to access one of the keys used as arguments";
    std::vector<std::string_view> keys;
    cmd_collect_keys(spec, cmd, keys);
    for (std::string_view k : keys) {
      if (!acl_key_allowed(u, k)) {
        return kdeny;
      }
    }
  }
...
bool acl_apply_rule(User &u, const std::string &t) {
  ...
  if (t == "allchannels" || t == "&*") {
    u.all_channels = true;
    u.channel_patterns.clear();
    return true;
  }
  if (t == "resetchannels") {
    u.channel_patterns.clear();
    u.all_channels = false;
    return true;
  }
  if (t.size() > 1 && t[0] == '&') {
  ...
  if (t == "allcommands" || t == "+@all") {
    u.allow_cats = CAT_ALL;
    u.cmd_overrides.clear();
    return true;
  }
  if (t == "nocommands" || t == "-@all") {
    u.allow_cats = 0;
    u.cmd_overrides.clear();
    return true;
  }
  ...
  if (t.size() > 1 && t[0] == '>') {
    ...
    if (h.empty()) {
      return false;
    } // entropy/KDF failure, reject failure
    u.pw_hashes.push_back(std::move(h));
    return true;
  }

  // <plain : remove any credential this plaintext matches. Salted PHC means
  // hash equality no longer works - verify per entry
  if (t.size() > 1 && t[0] == '<') {
    const std::string plain = t.substr(1);
    auto &v = u.pw_hashes;
    v.erase(std::remove_if(v.begin(), v.end(),
                           [&](const std::string &stored) {
                             return cred_verify(plain, stored);
                           }),
            v.end());
    return true;
  }

  // pre-hashed PHC credential (condig round-trip / ACL setuser  user
  // $argon2id@...)
  if (t.rfind("$argon2id$", 0) == 0) {
    if (t.size() > 512) {
      return false;
    } // sanity cap; garbage just never verifies
  ...
  if (t.rfind("+@", 0) == 0) {
    uint64_t b = acl_cat_bit(t.substr(2));
    if (!b) {
      return false;
    }
    u.allow_cats |= b;
...
std::string acl_format_user(const std::string &name, const User &u,
                            bool for_config) {
  std::string s = "user " + name + (u.enable ? " on" : " off");
  if (u.pw_hashes.empty()) {
    s += " nopass";
  } else {
    for (const std::string &h : u.pw_hashes) {
      if (!for_config) {
        s += " #<hash>";
      } // display: always redacted
      else if (h.rfind("$argon2id$", 0) == 0) {
        s += " \"" + h + "\"";
      } // PHC: verbatim token
      else {
        s += " \"#" + h + "\"";
  ...
  if (u.all_keys) {
    ...
    for (const std::string &p : u.key_patterns) {
      s += " ~" + p;
    }
  }
  ...
  if (u.allow_cats == CAT_ALL) {
    ...
    static const std::pair<uint64_t, const char *> cats[] = {
        {CAT_READ, "read"},
        {CAT_WRITE, "write"},
        {CAT_KEYSPACE, "keyspace"},
        {CAT_ADMIN, "admin"},
        {CAT_DANGEROUS, "dangerous"},
        {CAT_FAST, "fast"},
        {CAT_SLOW, "slow"},
...
static void do_acl(std::vector<std::string> &cmd, Buffer *out, Conn *conn) {
  ...
  if (sub == "users") {
    ...
    for (auto &kv : g_config.users) {
      resp_str(out, kv.first.data(), kv.first.size());
    }
    return;
  }

  if (sub == "cat") {
    static const char *cats[] = {"read",  "write",      "keyspace",
                                 "admin", "dangerous",  "fast",
                                 "slow",  "connection", "transaction"};
    resp_arr(out,
             (uint32_t)(sizeof(cats) / sizeof(cats[0]))); // resp array header
    for (const char *c : cats) {
      resp_str(out, c, strlen(c));
    }
    return;
  }
  ...
  if (sub == "genpass") {
    ...
    int nhex = (bits + 3) / 4;
    std::string s = cred_random_hex((size_t)nhex);
    if (s.empty()) {
      return resp_err(out, "ERR fail to generate a secure password");
    }
  ...
  if (sub == "setuser") {
    ...
    for (size_t i = 3; i < cmd.size(); ++i) {
      if (!acl_apply_rule(staged, cmd[i])) {
        return resp_err(
            out,
            ("ERR Error in ACL SETUSER modificer '" + cmd[i] + "'").c_str());
      }
    }
    if (staged.name.empty()) {
      staged.name = cmd[2];
  ...
  if (sub == "deluser") {
    if (cmd.size() < 3) {
      return resp_err(out, "ERR wrong number of arguments for 'acl|deluser'");
    }
    if (cmd[2] == "default") {
      return resp_err(out, "ERR the 'default' user cannot be removed");
    }
    auto it = g_config.users.find(cmd[2]);
  ...
  if (sub == "getuser") {
    if (cmd.size() < 3) {
      return resp_err(out, "ERR wrong number og arguments for 'acl|getuser'");
    }
    auto it = g_config.users.find(cmd[2]);
    if (it == g_config.users.end()) {
      return resp_nil(out);
...
// case insensitive subcommand compare againts a lowercase literal (was only use
// for the acls)
static bool arg_ieq(const std::string &s, const char *lit) {
...
// The count Redis reports in  every (p)subscribe/(p)unsubscribe confirmation is
// the conn's TOTAL subscription count - channels + patterns
// (clientSubscriptionCount)
static size_t pubsub_count(const Conn *conn) {
  return conn->sub_channels.size() + conn->sub_patterns.size();
}

// one "<kind> <channel> <count>" confirmation array (kind =
...
static void pubsub_confirm(Buffer *out, const char *kind,
  ...
  if (chan) {
    ...
    resp_nil(out);
  } // UNSUBSCRIBE with nothing subscribed
  resp_int(out, count);
}

// nullptr = allowed, else a NOPERM message
static const char *acl_channel_check(const User *u, const std::string &chan,
                                     bool literal) {
  static const char *deny =
      "NOPERM no permissions to access one of the channels used as arguments";
  if (!u) {
    return deny;
  }
  if (u->all_channels) {
...
static void do_subscribe(std::vector<std::string> &cmd, Buffer *out,
                         Conn *conn) {
...
static void pubsub_unlink(const std::string &ch, Conn *conn) {
  ...
  it->second.erase(conn);
  if (it->second.empty()) {
    g_data.channels.erase(it);
  }
}

static void do_unsubscribe(std::vector<std::string> &cmd, Buffer *out,
                           Conn *conn) {
  if (cmd.size() == 1) {
    // no argsL unsubscribe from everything currently held
    if (conn->sub_channels.empty()) {
      return pubsub_confirm(out, "unsubscribe", nullptr,
                            (int64_t)pubsub_count(conn));
    }
    // snapshot first: we mutate sub_channels while iterating
    std::vector<std::string> all(conn->sub_channels.begin(),
                                 conn->sub_channels.end());
  ...
  // explicit channels: reply for each arg even if we weren't subscribed (like
  // redis ":3")
  for (size_t i = 1; i < cmd.size(); ++i) {
    const std::string &ch = cmd[i];
    if (conn->sub_channels.erase(ch)) {
      pubsub_unlink(ch, conn);
    }
    pubsub_confirm(out, "unsubscribe", &ch, (int64_t)pubsub_count(conn));
  }
...
static void do_psubscribe(std::vector<std::string> &cmd, Buffer *out,
                          Conn *conn) {
  // pre-scan every argument
  for (size_t i = 1; i < cmd.size(); ++i) {
...
static void pubsub_punlink(const std::string &pat, Conn *conn) {
  ...
  if (it == g_data.patterns.end()) {
    return;
  }
  it->second.erase(conn);
  if (it->second.empty()) {
    g_data.patterns.erase(it);
  }
}

static void do_punsubscribe(std::vector<std::string> &cmd, Buffer *out,
                            Conn *conn) {
  if (cmd.size() == 1) {
    if (conn->sub_patterns.empty()) {
      return pubsub_confirm(out, "punsubscribe", nullptr,
                            (int64_t)pubsub_count(conn));
    }
    // snapshot: we mutate sub_patterns while walking it
    std::vector<std::string> all(conn->sub_patterns.begin(),
                                 conn->sub_patterns.end());
  ...
  for (size_t i = 1; i < cmd.size(); ++i) {
    const std::string &pat = cmd[i];
    if (conn->sub_patterns.erase(pat)) {
      pubsub_punlink(pat, conn);
    }
    pubsub_confirm(out, "punsubscribe", &pat, (int64_t)pubsub_count(conn));
  }
...
// core fan-out shared bu PUBLISH and keyspace notifications; return receiver
// count
static int64_t pubsub_publish(const std::string &chan, const char *msg,
                              size_t msglen) {
  int64_t receivers = 0;
  ...
  for (const auto &pe : g_data.patterns) {
    ...
    if (!glob_match(pat.data(), pat.size(), chan.data(), chan.size())) {
      continue;
    }
...
void notify_keyspace_event(int type, const char *event,
  ...
  if (flags & NOTIFY_KEYEVENT) {
    pubsub_publish(std::string("__keyevent@0__:") + event, key.data(),
                   key.size());
  }
}
...
void pubsub_remove_conn(Conn *conn) {
  for (const std::string &ch : conn->sub_channels) {
    pubsub_unlink(ch, conn);
  }
  conn->sub_channels.clear();
  for (const std::string &pat : conn->sub_patterns) {
    pubsub_punlink(pat, conn);
  }
  conn->sub_patterns.clear();
}

// MULTI/DISCARD need the conn, so do-request special-cases them by canonical
// name place holder again
static void do_txn_stub(std::vector<std::string> &, Buffer *) {}

static void do_multi(std::vector<std::string> &, Buffer *out, Conn *conn) {
  if (conn->in_multi) {
    return resp_err(out, "ERR MULTI calls can not be nested");
  }
  conn->in_multi = true;
...
static void do_discard(std::vector<std::string> &, Buffer *out, Conn *conn) {
  if (!conn->in_multi) {
    return resp_err(out, "ERR DISCARD without MULTI");
  }
  conn->in_multi = false;
  conn->multi_dirty = false;
  conn->queue_cmds.clear();
...
static void do_exec(std::vector<std::string> &, Buffer *out, Conn *conn) {
  if (!conn->in_multi) {
    return resp_err(out, "ERR EXEC without MULTI");
  }
  ...
  if (conn->multi_dirty) {
    ...
    conn->queue_cmds.clear();
    return resp_err(
        out, "EXECABORT Transaction discarded because of prevous errors.");
  }
...
static void do_watch(std::vector<std::string> &cmd, Buffer *out, Conn *conn) {
  ...
  if (conn->in_multi) {
    return resp_err(out, "ERR WATCH inside MULTI is not allowed");
  }
...
// WAIT <numreplicas> <timeout-ms>, how many replicas have applied everything
// written so far. never blocks the event loop
static void do_wait(std::vector<std::string> &cmd, Buffer *out, Conn *conn) {
  long num = 0, tmo = 0;
  if (!parse_int_strict(cmd[1].c_str(), &num) || num < 0) {
    return resp_err(out, "ERR value is not an integer or out of range");
  }
  if (!parse_int_strict(cmd[2].c_str(), &tmo) || tmo < 0) {
    return resp_err(out, "ERR timeout is not an integer or out of range");
  }

  // sampled before anything else can move it: this is the offset to catch up to
  const uint64_t target = g_data.master_repl_offset;
  const size_t reached = replicas_acked(target);

  // Answer on the spot when we already can
  if (reached >= (size_t)num || conn->in_exec || g_data.g_loading) {
    return resp_int(out, (int64_t)reached);
  }
  conn->wait_pending = true;
  conn->wait_target = target;
  conn->wait_numreplicas = (int)num;
  conn->wait_deadline_ms = tmo ? get_monotonic_msec() + (uint64_t)tmo : 0;
  g_data.waiters.insert(conn);
  // deliberately no reply: try_one_request stops parsing this conn until
  // resumed
}

static void do_acl_placeholder(std::vector<std::string> &, Buffer *) {
} // real dispact is conn-aware
...
struct DispatchEntry {
  std::string
      canonical; // original command name - used for ACL + AOF, never the alies
...
bool command_is_known(const std::string &name) {
  return k_cmd_table.count(name) != 0;
}

// build the frozen live dispatch map: canonical specs first, then apply
// rename-command
void dispatch_build() {
  g_dispatch.clear();
  ...
  for (const auto &r : g_config.renames) {
    ...
    // else disable
    if (!r.second.empty()) {
      g_dispatch.emplace(r.second, std::move(entry));
    }
  }
}
...
void metadata_selfcheck() {
  ...
  for (const auto &kv : k_cmd_table) {
    ...
    if (s.acl_cats == 0) {
      fprintf(stderr,
              "selfcheck: '%.*s' has acl_cats==0 (was acl_init_categories() "
              "run?)\n",
              (int)n.size(), n.data());
      problems++;
    }
    // control plane commands must not carry @read/@write
    if ((s.acl_cats & (CAT_ADMIN | CAT_DANGEROUS)) &&
        (s.acl_cats & (CAT_READ | CAT_WRITE))) {
      fprintf(stderr,
              "selfcheck: '%.*s' is admin/dangerous but still carries "
              "@read/@write\n",
              (int)n.size(), n.data());
      problems++;
    }
...
static bool cmd_can_grow_memory(const std::string &name) {
  ...
  static const std::unordered_set<std::string_view> no_grow{
      "del",      "unlink",    "flushall", "flushdb",  "expire", "pexpire",
...
static bool cmd_ok_in_subscribe(const std::string &canonical) {
  return canonical == "subscribe" || canonical == "unsubscribe" ||
         canonical == "psubscribe" ||
         canonical == "punsubscribe" || // this is for v8.2
...
void do_request(std::vector<std::string> &cmd, Buffer *out, Conn *conn,
  ...
  if (canonical == "discard") {
    return do_discard(cmd, out, conn);
  }
  if (canonical == "exec") {
    return do_exec(cmd, out, conn);
  }
  if (canonical == "watch") {
    return do_watch(cmd, out, conn);
  }
  if (canonical == "unwatch") {
    return do_unwatch(cmd, out, conn);
  }
  ...
  if (conn->in_multi) {
    if (cmd_no_queue(canonical)) {
      ...
      const std::string msg =
          "ERR " + canonical + " is not allowed in transactions";
      return resp_err(out, msg.c_str());
    }
  ...
  if (canonical == "unsubscribe") {
    return do_unsubscribe(cmd, out, conn);
  }
  if (canonical == "publish") {
    return do_publish(cmd, out, conn);
  }
  if (canonical == "psubscribe") {
    return do_psubscribe(cmd, out, conn);
  }
  if (canonical == "punsubscribe") {
    return do_punsubscribe(cmd, out, conn);
  }
  if (canonical == "replconf") {
    return do_replconf(cmd, out, conn);
  }
  if (canonical == "psync") {
    return do_psync(cmd, out, conn);
  }
  if (canonical == "wait") {
    return do_wait(cmd, out, conn);
  }

  // audit sensitive commands
  if (spec.acl_cats & (CAT_ADMIN | CAT_DANGEROUS)) {
    audit_event((spec.acl_cats & CAT_ADMIN) ? "admin_command"
                                            : "dangerous_command",
                conn, " cmd=" + canonical);
  }

  if (spec.is_write && g_config.aof_enable && g_data.g_aof_write_err &&
      !g_data.g_loading) {
    return resp_err(
        out, "MISCONF Errors writing to the AOF file, can't accept writes");
  }

  // maxmemory enforcement: make room before a write, or refuse it
  if (spec.is_write && cmd_can_grow_memory(canonical) &&
      !free_memory_if_needed()) {
    return resp_err(out,
                    "OOM commands not allowed when used memory > 'maxmemory'");
  }

  uint32_t dirty_before = g_data.g_writes_since_save;

  bool may_notify =
      spec.notify_class && g_config.notify_keyspace_events && !g_data.g_loading;
  std::string notify_key;
  if (may_notify && cmd.size() > 1) {
    notify_key = cmd[1];
  }

  // capture watched key names before spec.fn (it consumes the cmd)
  bool may_watch =
      spec.is_write && !g_data.watchers.empty() && !g_data.g_loading;
  std::vector<std::string> watch_keys;
  if (may_watch) {
    std::vector<std::string_view> kv;
    cmd_collect_keys(spec, cmd, kv);
    watch_keys.reserve(kv.size());
    for (std::string_view k : kv) {
      watch_keys.emplace_back(k);
    } // copy: cmd gets comsumed
  }
  ...
  if (may_watch && g_data.g_writes_since_save != dirty_before) {
    for (const std::string &k : watch_keys) {
      watch_touch(k);
    }
  }
  ...
  if (may_notify && !notify_key.empty() &&
  ...
  mem_selfcheck(
      canonical
          .c_str()); // prints "[mem] drift..." if any handler mis-accounted
#endif
}

static void kr_smove(const std::vector<std::string> &cmd,
                     std::vector<std::string_view> &keys) {
  if (cmd.size() > 1) {
    keys.emplace_back(cmd[1]);
  } // source
  ...
  } // destination - cmd[3] is a member not a key
}

static void kr_object(const std::vector<std::string> &cmd,
                      std::vector<std::string_view> &keys) {
  // OBJECT ENCODING|REFCOUNT|IDLETIME|FREQ key -> key at cmd[2];  HELP/STATS ->
  // no key
  if (cmd.size() > 2 &&
      (arg_ieq(cmd[1], "encoding") || arg_ieq(cmd[1], "refcount") ||
       arg_ieq(cmd[1], "idletime") || arg_ieq(cmd[1], "freq"))) {
    keys.emplace_back(cmd[2]);
  }
}

static void kr_memory(const std::vector<std::string> &cmd,
                      std::vector<std::string_view> &keys) {
...
// Populate CmdSpec::Acl_cats once at boot, the  @read/@write base is derived
// from the existing is_write flag, simplified taxonomy vs redis
void acl_init_categories() {
  ...
  static const std::unordered_map<std::string_view, KeySpec> ks = {
      ...
      {"del", KeySpec::ALL_FROM_1},
      {"unlink", KeySpec::ALL_FROM_1},
      {"exists", KeySpec::ALL_FROM_1},
      {"touch", KeySpec::ALL_FROM_1},
      {"mget", KeySpec::ALL_FROM_1},
      {"rename", KeySpec::ALL_FROM_1},
      {"renamenx", KeySpec::ALL_FROM_1},
      {"sinter", KeySpec::ALL_FROM_1},
      {"sunion", KeySpec::ALL_FROM_1},
      {"sdiff", KeySpec::ALL_FROM_1},
      {"sinterstore", KeySpec::ALL_FROM_1},
      {"sunionstore", KeySpec::ALL_FROM_1},
      {"sdiffstore", KeySpec::ALL_FROM_1},
      // key value key value ...
      {"mset", KeySpec::STRIDE2_FROM_1},
      {"msetnx", KeySpec::STRIDE2_FROM_1},
  ...
  static const std::unordered_map<std::string_view, uint64_t> extra = {
      ...
      {"save", CAT_ADMIN | CAT_DANGEROUS},
      {"bgsave", CAT_ADMIN | CAT_DANGEROUS},
      {"bgrewriteaof", CAT_ADMIN | CAT_DANGEROUS},
      {"flushall", CAT_KEYSPACE | CAT_DANGEROUS},
      {"flushdb", CAT_KEYSPACE | CAT_DANGEROUS},
      ...
      {"echo", CAT_CONNECTION},
      {"subscribe", CAT_FAST},
      {"unsubscribe", CAT_FAST},
      {"publish", CAT_FAST},
      {"psubscribe", CAT_FAST},
      {"punsubscribe", CAT_FAST},
      {"multi", CAT_FAST | CAT_TRANSACTION},
      {"discard", CAT_FAST | CAT_TRANSACTION},
      {"exec", CAT_SLOW | CAT_TRANSACTION},
      {"watch", CAT_FAST | CAT_TRANSACTION},
  ...
  static const std::unordered_map<std::string_view, int> notify_cls = {
      {"set", NOTIFY_STRING},       {"setnx", NOTIFY_STRING},
      {"setex", NOTIFY_STRING},     {"psetex", NOTIFY_STRING},
      {"getset", NOTIFY_STRING},    {"append", NOTIFY_STRING},
      {"setrange", NOTIFY_STRING},  {"incr", NOTIFY_STRING},
      {"decr", NOTIFY_STRING},      {"incrby", NOTIFY_STRING},
      {"decrby", NOTIFY_STRING},    {"incrbyfloat", NOTIFY_STRING},
      {"mset", NOTIFY_STRING},      {"msetnx", NOTIFY_STRING},
      {"del", NOTIFY_GENERIC},      {"unlink", NOTIFY_GENERIC},
      {"rename", NOTIFY_GENERIC},   {"renamenx", NOTIFY_GENERIC},
      {"getdel", NOTIFY_GENERIC},   {"getex", NOTIFY_GENERIC},
      {"expire", NOTIFY_GENERIC},   {"pexpire", NOTIFY_GENERIC},
      {"expireat", NOTIFY_GENERIC}, {"pexpireat", NOTIFY_GENERIC},
      {"persist", NOTIFY_GENERIC},  {"lpush", NOTIFY_LIST},
      {"rpush", NOTIFY_LIST},       {"lpop", NOTIFY_LIST},
      {"rpop", NOTIFY_LIST},        {"lset", NOTIFY_LIST},
      {"linsert", NOTIFY_LIST},     {"lrem", NOTIFY_LIST},
      {"ltrim", NOTIFY_LIST},       {"sadd", NOTIFY_SET},
      {"srem", NOTIFY_SET},         {"spop", NOTIFY_SET},
      {"smove", NOTIFY_SET},        {"sinterstore", NOTIFY_SET},
      {"sunionstore", NOTIFY_SET},  {"sdiffstore", NOTIFY_SET},
      {"hset", NOTIFY_HASH},        {"hdel", NOTIFY_HASH},
