...
def phase_aof_rewrite(ctx: "PhaseCtx"):
    ...
    _phase_aof_child_failure(ctx)


def _aof_child_helpers(srv, s):
    """Small readers shared by the three rewrite-child sections below."""
    def fd_targets():
        base = f"/proc/{srv.proc.pid}/fd"
        out = {}
        try:
        ...
        for f in names:
            try:
                out[f] = os.readlink(f"{base}/{f}")
            except OSError:
                pass
        return out

    def pinned_deleted_bytes():
        """Bytes of superseded AOF generations still held open. A rewrite that
        leaks its old fd compacts the file on paper only: the inode stays alive
        and the filesystem never gets the space back until the process exits."""
        base = f"/proc/{srv.proc.pid}/fd"
        total = 0
        for f, t in fd_targets().items():
            if t.endswith("appendonly.aof (deleted)"):
                try:
                    total += os.stat(f"{base}/{f}").st_size
                except OSError:
                    pass
        return total
    ...
    def rss_kb():
        try:
            with open(f"/proc/{srv.proc.pid}/status") as f:
                for line in f:
    ...
    def pending():
        return info_dict(s, "persistence").get("aof_pending_rewrite")

    def child_pids():
        return [int(p) for p in
                re.findall(r"aof_rewrite: started \(pid=(\d+)\)",
                           srv.stderr_text())]

    def wait_new_child(seen, budget=6.0):
        deadline = time.time() + budget
        while time.time() < deadline:
            got = [p for p in child_pids() if p not in seen]
            if got:
                return got[-1]
            time.sleep(0.05)
        return None

    def wait_idle(budget=6.0):
        return wait_until(lambda: pending() == "0", budget)

    return (fd_targets, pinned_deleted_bytes, rss_kb, pending, child_pids,
            wait_new_child, wait_idle)


# One burst is 100 MSETs of 40 x 8 KB values: ~32 MB of write stream in 100
# round trips rather than 4000.
MSET_BURST_PAIRS = 40
MSET_BURST_ROUNDS = 100
MSET_BURST_VAL = 8192
MSET_BURST_KB = (MSET_BURST_ROUNDS * MSET_BURST_PAIRS
                 * (MSET_BURST_VAL + 48)) // 1024


def _mset_burst(sock, prefix, rss_kb):
    """Write one burst; return the RSS it cost, in KB."""
    val = "v" * MSET_BURST_VAL
    before = rss_kb()
    for r in range(MSET_BURST_ROUNDS):
        args = []
        for i in range(MSET_BURST_PAIRS):
            args += [f"{prefix}:{r}:{i}", val]
        cmd(sock, "MSET", *args)
    time.sleep(0.7)
    return rss_kb() - before


def _aof_child_conf(ctx, tag):
    port = ctx.port()
    d = ctx.dir(tag)
    conf = write_conf(os.path.join(d, "srv.conf"), [
        f"port {port}",
        ...
        "appendfilename appendonly.aof",
        'save ""',
        # Only the rewrites these tests ask for: a spontaneous one could occupy
        # the single child slot in the middle of a measurement.
        "auto-aof-rewrite-percentage 0",
    ])
    return port, d, conf


def _phase_aof_child_failure(ctx: "PhaseCtx"):
    """What the parent does when a rewrite child does NOT exit cleanly, and what
    it does with fds when one exits perfectly.

    All of it is aof_rewrite_reap(). A fork-based rewrite fails in production
    two ways — the OOM killer picks the child (it is the newest large RSS on the
    box) or the disk fills and the child _exit(1)s — and neither is exotic
    enough to leave untested.

    The child is frozen deterministically rather than raced: aof_write_snapshot
    opens a FIXED path (<aof>.tmp) with O_WRONLY|O_CREAT|O_TRUNC, and opening a
    FIFO for write blocks until a reader arrives. That is exactly what "still
    writing" looks like from the parent's side, held for as long as the test
    needs, with no sleep-and-hope.

    Each of the three sections runs its own instance. They shared one at first,
    and a parent left wedged by the signal case then made the two cases after it
    silently unable to fork a child at all — checks that neither passed nor
    failed, which is the one outcome a suite must never produce.
    """
    ctx.section("Persistence: rewrite-child failure and fd hygiene")
    if not os.path.isdir("/proc/self/fd"):
        ctx.skip("rewrite-child failure", "no /proc, so fds cannot be counted")
        return
    _aof_rewrite_fd_hygiene(ctx)
    _aof_child_killed(ctx)
    _aof_child_nonzero_exit(ctx)


def _aof_rewrite_fd_hygiene(ctx: "PhaseCtx"):
    """A rewrite that succeeds perfectly still has to give back what it used."""
    port, d, conf = _aof_child_conf(ctx, "aof-fd-hygiene")
    aof = os.path.join(d, "appendonly.aof")
    srv = ctx.start("aof-fd-hygiene", d, conf, port)
    s = raw_conn(port)
    (_, pinned_deleted_bytes, _, _, _, _, wait_idle) = _aof_child_helpers(srv, s)
    try:
        # A file worth compacting: 400 keys written, 390 deleted. The rewrite
        # should turn ~400 KB of log into a few hundred bytes.
        for i in range(400):
            cmd(s, "SET", f"fd:{i}", "v" * 1024)
        cmd(s, "DEL", *[f"fd:{i}" for i in range(390)])
        _aof_settled(s, aof, "fdleak")
        before_fds = len(os.listdir(f"/proc/{srv.proc.pid}/fd"))
        before_bytes = os.path.getsize(aof)

        for _ in range(3):
            cmd(s, "BGREWRITEAOF")
            if not wait_idle(15.0):
                break
            cmd(s, "SET", "fd:churn", "x")
        after_fds = len(os.listdir(f"/proc/{srv.proc.pid}/fd"))

        ctx.ok("[REG] a successful rewrite closes the AOF fd it replaced",
               after_fds <= before_fds,
               f"fd count {before_fds} -> {after_fds} over 3 rewrites: the reap "
               f"closes g_aof_current_size — a byte count — instead of "
               f"g_aof_fd, so the live fd is leaked every single time")
        pinned = pinned_deleted_bytes()
        ctx.ok("[REG] a rewrite frees the file it compacted",
               pinned == 0,
               f"{pinned} bytes of superseded AOF are still held open by a "
               f"leaked fd, so the file shrinking {before_bytes} -> "
               f"{os.path.getsize(aof)} freed no space at all")
    finally:
        try:
            s.close()
        except OSError:
...
def _aof_shadow_control(ctx: "PhaseCtx"):
    """The same instance shape, the same burst, but a rewrite that COMPLETES.

    A control instance rather than a threshold, because no absolute RSS number
    is portable across builds: this burst costs ~0 KB on Release (the ~32 MB
    buffer rdb_build_aof_preamble just built and freed is arena enough to hold
    the whole keyspace copy) and ~112 MB under ASan, whose size classes and
    redzones dwarf the thing being measured. Both numbers move by roughly one
    payload when the duplicate is there, so the comparison has to be against a
    server built the same way that got as far as the same rewrite."""
    port, d, conf = _aof_child_conf(ctx, "aof-shadow-control")
    srv = ctx.start("aof-shadow-control", d, conf, port)
    s = raw_conn(port)
    (_, _, rss_kb, _, _, _, wait_idle) = _aof_child_helpers(srv, s)
    try:
        for i in range(20):
            cmd(s, "SET", f"seed:{i}", "x")
        cmd(s, "BGREWRITEAOF")
        if not wait_idle(20.0):
            return None
        return _mset_burst(s, "shadow", rss_kb)
    finally:
        try:
            s.close()
        except OSError:
...
def _aof_child_killed(ctx: "PhaseCtx"):
    """The OOM killer's usual target is exactly this fork."""
    port, d, conf = _aof_child_conf(ctx, "aof-child-killed")
    tmp = os.path.join(d, "appendonly.aof.tmp")
    srv = ctx.start("aof-child-killed", d, conf, port)
    s = raw_conn(port)
    (_, _, rss_kb, pending, child_pids,
     wait_new_child, wait_idle) = _aof_child_helpers(srv, s)
    frozen = []
    try:
        for i in range(20):                 # a deliberately tiny seed: see below
            cmd(s, "SET", f"seed:{i}", "x")

        seen = set(child_pids())
        os.mkfifo(tmp)
        mark = len(srv.stderr_text())
        cmd(s, "BGREWRITEAOF")
        child = wait_new_child(seen)
        if child is None:
            ctx.ok("a rewrite child could be caught mid-write", False,
                   f"stderr tail: {srv.stderr_tail(6)}")
            return
        frozen.append(child)

        child_socks = []
        try:
            for f in os.listdir(f"/proc/{child}/fd"):
                if "socket:" in os.readlink(f"/proc/{child}/fd/{f}"):
                    child_socks.append(f)
        except OSError:
            pass
        ctx.ok("[REG] the rewrite child does not inherit the server's sockets",
               not child_socks,
               f"the child holds {len(child_socks)} socket fds ({child_socks}): "
               f"nothing in server.cpp uses SOCK_CLOEXEC or accept4, so a client "
               f"that hangs up mid-rewrite cannot finish closing until the child "
               f"exits, and the listener stays bound after the parent is gone")

        os.kill(child, signal.SIGKILL)
        time.sleep(1.0)
        logged = srv.stderr_text()[mark:].replace(
            f"aof_rewrite: started (pid={child})", "")
        ctx.ok("[REG] a rewrite child killed by a signal is reported",
               "aof_rewrite" in logged,
               f"nothing was logged between the fork and now: "
               f"{logged.strip().splitlines() or '(silence)'} — a rewrite that "
               f"died is indistinguishable from one still running")

        # The shadow copy, measured BEFORE anything un-wedges the parent.
        #
        # propagate() mirrors every write into g_aof_rewrite_buf while a child is
        # believed to be running, and with the pid stuck nothing ever drains it.
        # The seed is deliberately tiny: an earlier version wrote 16 MB first,
        # and the buffer rdb_build_aof_preamble had just freed was large enough
        # to absorb the entire duplicate — that run measured the allocator and
        # pronounced a broken server clean.
        shadow_kb = _mset_burst(s, "shadow", rss_kb)
        control_kb = _aof_shadow_control(ctx)
        extra = shadow_kb - control_kb
        ctx.ok("[REG] a failed rewrite does not shadow-copy every later write",
               control_kb is not None and extra < MSET_BURST_KB * 0.5,
               f"the identical {MSET_BURST_KB} KB workload cost {control_kb} KB "
               f"of RSS on a server whose rewrite finished and {shadow_kb} KB "
               f"here — {extra} KB more, about one whole extra copy of the "
               f"write stream sitting in g_aof_rewrite_buf with nothing left "
               f"to drain it")

        ctx.ok("[REG] a killed rewrite child clears aof_pending_rewrite",
               wait_idle(),
               f"INFO aof_pending_rewrite is still {pending()!r}: "
               f"aof_rewrite_reap() only reaches g_aof_child_pid = -1 inside "
               f"its success branch, so waitpid() has already reaped a process "
               f"the server still believes is running")

        if os.path.exists(tmp):
            os.unlink(tmp)      # the freeze, not the bug, would block the next one
        seen = set(child_pids())
        cmd(s, "BGREWRITEAOF")
        ctx.ok("[REG] a later BGREWRITEAOF really runs after a failed one",
               wait_new_child(seen) is not None,
               "no new child was forked: the stale pid makes "
               "aof_rewrite_background() bail at its 'already running' guard "
               "while BGREWRITEAOF still replies success to the client")
        wait_idle(15.0)
        ctx.ok("the server still serves writes after a failed rewrite",
               cmd(s, "SET", "after:kill", "ok") == "OK" and
               cmd(s, "GET", "after:kill") == "ok")
    finally:
        for p in frozen:
            try:
...
def _aof_child_nonzero_exit(ctx: "PhaseCtx"):
    """The other production failure: the disk the child writes to is full or
    read-only, so it never gets past open() and _exit(1)s."""
    port, d, conf = _aof_child_conf(ctx, "aof-child-exit1")
    tmp = os.path.join(d, "appendonly.aof.tmp")
    srv = ctx.start("aof-child-exit1", d, conf, port)
    s = raw_conn(port)
    (_, _, _, pending, child_pids, wait_new_child, wait_idle) = \
        _aof_child_helpers(srv, s)
    try:
        for i in range(20):
            cmd(s, "SET", f"seed:{i}", "x")
        os.mkdir(tmp)                      # open() fails -> the child _exit(1)s
        seen = set(child_pids())
        mark = len(srv.stderr_text())
        cmd(s, "BGREWRITEAOF")
        child = wait_new_child(seen)
        if child is None:
            ctx.ok("a rewrite child could be caught for the exit(1) case", False,
                   f"stderr tail: {srv.stderr_tail(6)}")
            return
        ctx.ok("[REG] a rewrite child that exits non-zero clears the pid too",
               wait_idle(),
               f"INFO aof_pending_rewrite is {pending()!r} after the child "
               f"failed to open its temp file — the same success-only exit path "
               f"as the signal case, reached a different way")
        ctx.ok("[REG] a rewrite child that exits non-zero is reported",
               "aof_rewrite" in srv.stderr_text()[mark:].replace(
                   f"aof_rewrite: started (pid={child})", ""),
               f"logged since the fork: "
               f"{srv.stderr_text()[mark:].strip().splitlines() or '(silence)'}")
    finally:
...
# ── keys that are legal but do not look like text ────────────────────────────
#
# RESP bulk strings are length-prefixed, so a key may legally contain CR, LF,
# NUL, a quote, or a byte that looks like the start of a new frame. The parser
# is not the interesting question — it never scans for a delimiter. The question
# is whether anything DOWNSTREAM re-encodes such a key as if it were
# delimiter-bounded: the AOF rewrite re-emits every key, and the RDB writes and
# re-reads them. Both are checked here, and both are clean; the place that does
# get it wrong is CONFIG REWRITE (see the config phase).
...
def _phase_binary_keys(ctx: "PhaseCtx"):
    ctx.section("Persistence: keys holding control bytes survive a re-encode")
    port = ctx.port()
    d = ctx.dir("binary-keys")
    conf = write_conf(os.path.join(d, "srv.conf"), [
        f"port {port}", "appendonly yes", "appendfilename appendonly.aof",
        "dbfilename dump.rdb", 'save ""', "auto-aof-rewrite-percentage 0",
    ])
    aof = os.path.join(d, "appendonly.aof")
    srv = ctx.start("binary-keys", d, conf, port)
    s = raw_conn(port)
    try:
        for name, k in NASTY_KEYS.items():
            cmd(s, "SET", k, "v-" + name)
            cmd(s, "LPUSH", "list-" + k, "e-" + name)
            cmd(s, "HSET", "hash-" + k, k, "hv-" + name)
        want = 3 * len(NASTY_KEYS)
        ctx.ok("every control-byte key was stored", cmd(s, "DBSIZE") == want,
               f"DBSIZE={cmd(s, 'DBSIZE')} expected {want}")

        cmd(s, "BGREWRITEAOF")
        wait_until(lambda: info_dict(s, "persistence").get(
            "aof_pending_rewrite") == "0", 20.0)
        _aof_settled(s, aof, "binkeys")
        s.close()
        srv = ctx.restart(srv, "binary-keys-aof")
        s = raw_conn(port)
        ctx.ok("[REG] the AOF rewrite re-encoded every control-byte key",
               cmd(s, "DBSIZE") == want,
               f"DBSIZE={cmd(s, 'DBSIZE')} after replay, expected {want} — a "
               f"key truncated at a CR or LF comes back as a different key")
        bad = [n for n, k in NASTY_KEYS.items()
               if cmd(s, "GET", k) != "v-" + n]
        ctx.ok("[REG] every value came back intact through the AOF", not bad,
               f"wrong after replay: {bad}")

        cmd(s, "CONFIG", "SET", "appendonly", "no")
        cmd(s, "SAVE")
        s.close()
        srv = ctx.restart(srv, "binary-keys-rdb")
        s = raw_conn(port)
        ctx.ok("[REG] the RDB round-trip kept every control-byte key",
               cmd(s, "DBSIZE") == want,
               f"DBSIZE={cmd(s, 'DBSIZE')} after the RDB load, expected {want}")
        bad = [n for n, k in NASTY_KEYS.items()
               if cmd(s, "GET", k) != "v-" + n]
        ctx.ok("[REG] every value came back intact through the RDB", not bad,
               f"wrong after the RDB load: {bad}")
    finally:
        try:
            s.close()
        except OSError:
...
def _getuser_dict(sock, name):
    """ACL GETUSER as a dict. The reply is a flat [k, v, k, v, ...] array."""
    r = cmd(sock, "ACL", "GETUSER", name)
    if not isinstance(r, list):
        return {}
    return {r[i]: r[i + 1] for i in range(0, len(r) - 1, 2)}


def _security_acl_rules_mean_what_they_say(ctx: "PhaseCtx"):
    """Two properties an ACL implementation cannot be allowed to break:

      - a rule the server ACCEPTED is a rule the server ENFORCES. acl_apply_rule
        validates `+@cat`/`-@cat` against the category table and rejects an
        unknown one, but takes any string at all after a bare '+' or '-' and
        files it under that name. acl_check only ever looks up the CANONICAL
        command name, so a typo, or the alias an operator actually types after a
        rename-command, is stored, listed back by ACL LIST, and never consulted.

      - the introspection command agrees with the enforcement. ACL GETUSER
        renders `commands` as one of three fixed strings and never reads
        cmd_overrides or the channel patterns at all.

    Its own instance: proving enforcement means actually running the command
    under test, and the natural candidate in the security phase is FLUSHALL.
    """
    ctx.section("Security: an ACL rule must mean what it says")
    port = ctx.port()
    d = ctx.dir("acl-semantics")
    conf = write_conf(os.path.join(d, "srv.conf"), [
        f"port {port}",
        f'requirepass "{SEC_ADMIN_PW}"',
        "appendonly no",
        'save ""',
        "rename-command get zzget",      # harmless to run, unlike flushall
    ])
    srv = ctx.start("acl-semantics", d, conf, port)
    a = raw_conn(port, SEC_ADMIN_PW)
    try:
        # 1. a name that is not a command at all
        err = err_of(a, "ACL", "SETUSER", "typo", "on", ">tp", "~*", "+@all",
                     "-nosuchcommand")
        ctx.ok("[REG] an unknown command name in an ACL rule is rejected",
               err is not None,
               "SETUSER accepted '-nosuchcommand' and stored it under a name "
               "acl_check will never look up: the deny is listed back by ACL "
               "LIST and does nothing. The same parser rejects an unknown "
               "category one branch away")

        # 2. the alias of a renamed command: the name an operator types
        err = err_of(a, "ACL", "SETUSER", "alias", "on", ">al", "~*", "+@all",
                     "-zzget")
        enforced = None
        if err is None:
            u = raw_conn(port, "al", user="alias")
            enforced = err_of(u, "zzget", "any:key") is not None
            u.close()
        ctx.ok("[REG] an ACL rule naming a renamed alias is rejected or enforced",
               err is not None or enforced,
               "'-zzget' was accepted and then ignored: the user it was written "
               "to deny still runs the command. Whether the fix is to reject "
               "the alias or to resolve it, silently keeping a dead rule is the "
        ...
        ctx.ok("[REG] ACL GETUSER shows a command granted on top of -@all",
               ...
               f"GETUSER reports commands={g.get('commands')!r} for a user that "
               f"can run PING — an audit of a single user cannot see any grant "
               f"carved out of the base rule (ACL LIST does show it)")

        cmd(a, "ACL", "SETUSER", "carve", "on", ">cv", "~*", "+@all", "-ping")
        g = _getuser_dict(a, "carve")
        ctx.ok("[REG] ACL GETUSER shows a command denied on top of +@all",
               "ping" in str(g.get("commands", "")).lower(),
               f"GETUSER reports commands={g.get('commands')!r} for a user that "
               f"is denied PING")

        # 5. channels are a permission too
        cmd(a, "ACL", "SETUSER", "chan", "on", ">ch", "~*", "+@all",
            "resetchannels", "&news:*")
        g = _getuser_dict(a, "chan")
        ctx.ok("[REG] ACL GETUSER reports the user's channel patterns",
               "news:*" in " ".join(str(v) for v in g.values()),
               f"GETUSER answers {g!r} — the channel grant is missing entirely, "
               f"so a pub/sub permission cannot be audited per user")

        # The other half of the same mechanism, and the one that checked out
        # clean: do_request resolves the typed name to its canonical before any
        # ACL check runs, so enforcement itself is not alias-sensitive. Worth
        # holding in place — this canonicalization path has already produced two
        # separate bugs elsewhere (the AOF-restart brick and the V9.5.1
        # admin-category mistagging).
        cmd(a, "ACL", "SETUSER", "nore", "on", ">nr", "~*", "+@write",
            "+@connection")
        u = raw_conn(port, "nr", user="nore")
        ctx.ok("a category deny still applies when the command is renamed",
               err_of(u, "zzget", "any:key") is not None,
               "a user without @read read a key through the renamed alias: the "
               "ACL check saw the alias instead of the canonical name")
        u.close()

        cmd(a, "ACL", "SETUSER", "scoped", "on", ">sc", "~ok:*", "+@all")
        u = raw_conn(port, "sc", user="scoped")
        ctx.ok("a key pattern still applies when the command is renamed",
               err_of(u, "zzget", "ok:1") is None and
               err_of(u, "zzget", "denied:1") is not None,
               "the key-pattern check disagrees with itself depending on "
               "whether the command was reached by its canonical name")
        u.close()
    finally:
        try:
            a.close()
...
def phase_security(ctx: "PhaseCtx"):
    ...
    _security_acl_rules_mean_what_they_say(ctx)
...
def phase_config_roundtrip(ctx: "PhaseCtx"):
    ...
    _phase_config_injection(ctx)


# ── operator strings that have to survive a line-oriented reader ──────────────
#
# CONFIG REWRITE is the one place a runtime-supplied string is written back into
# a file that the next boot parses line by line. config_write_scalar() quotes a
# value containing whitespace or a quote and escapes '"' and '\' — which is
# right for spaces and quotes and cannot work for a newline, because the newline
# ends the line before the reader ever sees the closing quote. Three of the emit
# lambdas skip even that much: `auditlog`, `requirepass` on its $argon2id$
# passthrough, and `user` via acl_format_user(), which writes the username bare.
#
# Every case below asserts on the ATTACK, not on any particular fix: rejecting
# the value at CONFIG SET time and escaping it at write time both pass, and
# both leave the restarted server carrying only the directives its operator set.
# `rename-command`'s setter already rejects control characters — the check these
# were all missing exists in the codebase exactly once.

CFG_INJECT_MARK = 20077          # a maxclients no test config ever sets


def _config_injection_case(ctx: "PhaseCtx", tag: str, arm, verify):
    """arm(sock) plants the payload; verify(ctx, sock, label) runs after the
    rewrite and restart. A payload the server refuses outright is a pass."""
    port = ctx.port()
    d = ctx.dir(f"cfg-inject-{tag}")
    path = os.path.join(d, "srv.conf")
    write_conf(path, [
        f"port {port}", f'requirepass "{CFG_PW}"', "appendonly no", 'save ""',
    ])
    srv = ctx.start(f"cfg-inject-{tag}", d, path, port)
    s = raw_conn(port, CFG_PW)
    try:
        refused = arm(s)
        cmd(s, "CONFIG", "REWRITE")
        s.close()
        try:
            srv = ctx.restart(srv, f"cfg-inject-{tag}-restart")
        except RuntimeError as e:
            ctx.ok(f"[REG] {tag}: the server still boots after CONFIG REWRITE",
                   False,
                   f"{e} — the rewritten config file is what stopped it; "
                   f"written by:\n      "
                   + "\n      ".join(open(path).read().splitlines()[-6:]))
            return
        ctx.ok(f"[REG] {tag}: the server still boots after CONFIG REWRITE", True)
        try:
            s = raw_conn(port, CFG_PW, timeout=4.0)
        except (RespError, OSError) as e:
            ctx.ok(f"[REG] {tag}: the operator's password still works", False,
                   f"{type(e).__name__}: {e} — the rewrite changed or truncated "
                   f"requirepass, so the server came back up with nobody able "
                   f"to log in")
            return
        ctx.ok(f"[REG] {tag}: the operator's password still works", True)
        verify(ctx, s, tag)
        if refused:
            print(f"      ({tag} was refused at set time: {refused})")
    except RespError as e:
        ctx.ok(f"[REG] {tag}: the round-trip completed", False, str(e))
    finally:
        try:
            s.close()
        except OSError:
...
def _no_injected_directive(ctx, s, tag):
    got = get_directive(s, "maxclients")
    ctx.ok(f"[REG] {tag}: no directive was injected into the config file",
           str(got) != str(CFG_INJECT_MARK),
           f"maxclients is {got!r} after the restart — a newline inside the "
           f"value ended the line, and the rest of it became a directive of "
           f"its own that this server is now running with")


def _phase_config_injection(ctx: "PhaseCtx"):
    ctx.section("Config: operator strings must not rewrite the config file")

    # Each payload ends in a '#' so that whatever the writer appends after the
    # injected line — a stray closing quote, the rest of an ACL rule — becomes a
    # comment. Without it the second line is malformed and the server merely
    # refuses to boot; with it the injected directive parses and takes effect,
    # which is the sharper claim and the one worth asserting against.
    def arm_auditlog(s):
        return err_of(s, "CONFIG", "SET", "auditlog",
                      f'audit.log"\nmaxclients {CFG_INJECT_MARK}  # ')

    def arm_dbfilename(s):
        return err_of(s, "CONFIG", "SET", "dbfilename",
                      f"dump.rdb\nmaxclients {CFG_INJECT_MARK}  # ")

    def arm_requirepass(s):
        # The $argon2id$ branch stores the value verbatim: a prefix is the whole
        # validation. Anything after it lands in the file unchanged.
        return err_of(s, "CONFIG", "SET", "requirepass",
                      f"$argon2id$x\nmaxclients {CFG_INJECT_MARK}  # ")

    def arm_acl_newline(s):
        return err_of(s, "ACL", "SETUSER",
                      f"bob\nmaxclients {CFG_INJECT_MARK}  # ", "on", ">bobpw",
                      "~x:*", "+@read", "+@connection")

    def arm_acl_space(s):
        # The name is written bare by acl_format_user(), so whitespace in it
        # becomes further ACL rules on the next boot — and `~*` widens the
        # user's key scope, because the `~pattern` branch never clears all_keys.
        return err_of(s, "ACL", "SETUSER", "bob on ~* +@read", "on", ">bobpw",
                      "~x:*", "+@read", "+@connection")

    def verify_acl_scope(ctx, s, tag):
        _no_injected_directive(ctx, s, tag)
        names = " ".join(cmd(s, "ACL", "LIST") or [])
        widened = []
        for line in (cmd(s, "ACL", "LIST") or []):
            if line.startswith("user bob") and "~*" in line.split():
                widened.append(line)
        ctx.ok(f"[REG] {tag}: the user's key scope is still what was granted",
               not widened,
               f"a user that was granted ~x:* came back with ~*: {widened} "
               f"(full listing: {names})")

    _config_injection_case(ctx, "auditlog", arm_auditlog, _no_injected_directive)
    _config_injection_case(ctx, "dbfilename", arm_dbfilename,
                           _no_injected_directive)
    _config_injection_case(ctx, "requirepass", arm_requirepass,
                           _no_injected_directive)
    _config_injection_case(ctx, "acl-username-newline", arm_acl_newline,
                           _no_injected_directive)
    _config_injection_case(ctx, "acl-username-space", arm_acl_space,
                           verify_acl_scope)
...
def phase_tls(ctx: "PhaseCtx"):
    ...
    try:
        ...
        # --- everything that is not a ClientHello ---------------------------
        #
        # A TLS listener is reachable by anything that can open a socket, so the
        # shapes below all arrive in practice: a client pointed at the wrong
        # port, a health checker speaking HTTP, a scanner sending a header and
        # leaving. Each has to end at the same clean tr_close() — closed
        # promptly or reaped at the deadline, never hung past it, and never
        # taking another session down with it. The bystander opened first is the
        # control.
        bystander = _tls_conn(hs_tls)
        ctx.ok("a healthy TLS session is up before the abuse",
               cmd(bystander, "PING") == "PONG")

        def _ends_within(payload: bytes, budget: float):
            """Send `payload` at the tls-port; return (ended, seconds)."""
            k = socket.socket()
            k.settimeout(budget)
            try:
                k.connect(("127.0.0.1", hs_tls))
                k.sendall(payload)
                t0 = time.time()
                try:
                    return k.recv(4096) == b"", time.time() - t0
        ...
        budget = 4.0        # 4x the configured handshake timeout
        for label, payload in (
                ("a plaintext RESP frame", b"*1\r\n$4\r\nPING\r\n"),
                ("an HTTP request", b"GET / HTTP/1.1\r\nHost: x\r\n\r\n"),
                ("a truncated ClientHello then silence",
                 bytes.fromhex("1603030020010000")),
                ("a record header claiming 16 KB that never arrives",
                 bytes.fromhex("16030140000100") + b"\x00" * 64)):
            ended, took = _ends_within(payload, budget)
            ctx.ok(f"[REG] {label} at the tls-port ends at the same clean close",
                   ended,
                   f"still open after {took:.1f}s against a 1s handshake "
                   f"timeout — a peer that never sends a ClientHello is holding "
                   f"a connection slot and its buffers")

        # the mirror: TLS material at the plaintext port
        raw = socket.create_connection(("127.0.0.1", hs_plain), TIMEOUT_SEC)
        try:
            _bare_tls_ctx().wrap_socket(raw, server_hostname=None)
            wrapped = True
            ...
            try:
                ...
                pass
        ctx.ok("[REG] a TLS handshake at the plaintext port fails cleanly",
               not wrapped,
               "the plaintext listener completed a TLS handshake, which means "
               "the two listeners are not as separate as the config says")

        ctx.ok("the bystander TLS session survived all of it",
               cmd(bystander, "PING") == "PONG",
               "one malformed peer took an unrelated TLS session with it")
        bystander.close()
        ctx.ok("a fresh TLS handshake still completes afterwards",
               cmd(_tls_conn(hs_tls), "PING") == "PONG")
        ctx.ok("the plaintext listener still serves afterwards",
               cmd(raw_conn(hs_plain), "PING") == "PONG")
    finally:
        hs_srv.stop()
