...
runnable regression suite means testing it by hand forever. That gate is met —
V11 closed 2026-08-22 — but **V13 was promoted ahead of this on the same day**:
pointing one real application at a single node is cheaper than sharding, and it
answers whether any of this is usable before the state space is multiplied.

## V13 - Production-Pointable: closing the app-compatibility gap

**Moved to `ROADMAP.md` → Current Focus on 2026-08-22** when V11 closed, and
promoted **ahead of V12 (cluster)**. The scoping work — Step 0's two uncertain
blockers, the priority ordering for the command-surface gaps, and the
deployment-ergonomics list — lives there now rather than being duplicated here.

The items it *points at* are still described in this file and stay here:
**Multiple Logical Databases**, **Command Coverage Gaps**, **ACL and
Command-Surface Feature Gaps**, **Scripting (EVAL)**, **Structured Logging and
Daemonization**, and **Server Observability and Tooling**. V13 adds priority,
not a second copy of any list — this project has been bitten more than once by
one command list being edited while a parallel copy was forgotten.
