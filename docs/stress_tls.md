# MYRED stress test — 2026-07-21 13:34:02

```
(logging output to docs/stress_tls.md)
═══════════════════════════════════════════════════════
  Redis Server RESP Stress Test
  Connecting to 127.0.0.1:1235
...
── CONFIG ────────────────────────────────────────────
  ...
  ✓ config get * -> array → ['maxmemory', '0', 'maxmemory-policy', 'allkeys-lru']
...
── ACL: users, auth, key patterns ────────────────────
  ...
  ✓ acl list -> array → ['user default on #<hash> ~* +@all']
...
── BGSAVE (fork-based background save) ───────────────
  ...
  ℹ  bgsave returned in 2.2ms: 'Background saving started'
  ✓ server responsive during save
  ℹ  100 ops during save took 18.3ms
...
═══════════════════════════════════════════════════════
Results: 555/555 passed
Runtime: 5.95s (93.3 assertions/sec)
All tests passed!
Slowest sections:
  0.92s  2/2  Memory: incremental eviction (EVICT_RUNNING semantics)
  0.77s  9/9  UNLINK Command (async delete)
  0.70s  9/9  Memory: maxmemory eviction + OOM
  0.61s  10/10  TTL Commands: PEXPIRE / PTTL
  0.57s  6/6  Persistence Round-trip (in-memory)
...
── Stress Test ────────────────────────────────────────
  ...
  Latency avg: 26.56ms
  Latency min: 0.13ms
  Latency max: 614.05ms
  Latency p50: 4.37ms
  Latency p95: 79.68ms
  Latency p99: 505.82ms
  No errors!
  Operation mix:
    append                119 ok     0 errors
    zrevquery             117 ok     0 errors
    info                  116 ok     0 errors
    keys                  115 ok     0 errors
    rpush                 114 ok     0 errors
    object_encoding       112 ok     0 errors
    ttl_triplet           112 ok     0 errors
    hscan                 111 ok     0 errors
    zscore                110 ok     0 errors
    hset                  109 ok     0 errors
    srem                  109 ok     0 errors
    list_pop_trim         109 ok     0 errors
  Slowest operations by average latency:
    keys                 413.48ms avg over 115 ops
    keyspace_scan        283.84ms avg over 98 ops
    hscan                 35.71ms avg over 111 ops
    hgetall               34.71ms avg over 96 ops
    sscan                 28.68ms avg over 91 ops
    smembers              27.47ms avg over 94 ops
    zrevquery             15.99ms avg over 117 ops
    zquery                12.21ms avg over 92 ops
    lrange                11.75ms avg over 102 ops
    list_pop_trim         10.30ms avg over 109 ops
    srandmember            9.71ms avg over 104 ops
    ttl_triplet            9.31ms avg over 112 ops
  ℹ  cleaned 162 leftover keys

-- Command Metrics -------------------------------------
  Commands observed: 17908
  RESP errors:       105 (expected negative tests included)
  Transport errors:  0
  Latency avg:       6.18ms
  Latency p50/p95/p99: 0.19/10.35/218.85ms
  Latency max:       614.03ms
  Most used commands:
    set              9324 calls
    zadd             1798 calls
    get               798 calls
    del               680 calls
    srandmember       360 calls
    rpush             328 calls
    sadd              235 calls
    hset              225 calls
    info              150 calls
  ...
  Slowest commands by average latency:
    keys             406.41ms avg over 117 calls
    scan             265.05ms avg over 101 calls
    hscan             34.46ms avg over 115 calls
    hgetall           33.99ms avg over 98 calls
    sscan             27.47ms avg over 95 calls
    smembers          25.57ms avg over 101 calls
    zrevquery         15.71ms avg over 119 calls
    zquery            11.46ms avg over 98 calls
    lrange            10.61ms avg over 113 calls
    mget               8.75ms avg over 101 calls
    zpopmin            8.26ms avg over 111 calls
    save               8.07ms avg over 2 calls
